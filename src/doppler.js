  ...
  var listenerTone = new Tone(
    {
       ...
       */
      radians: function () {
        // scaling up the velocity here to exaggerate doppler effect w/o having the image move across
        // the screen too quickly
        var sourceVelocityMulitplier = 30;
        return DUtils.freq2Rad(
          DMath.observedFrequency(
            DMath.radialVelocity(src.pvector(),
                                 sourceVelocityMulitplier * srcMover.velocity,
                                 listener.pvector()),
...
draw = function() {
  ...
  // passing `0` to prevent waveforms from animating, highlighting the change in observed frequency
